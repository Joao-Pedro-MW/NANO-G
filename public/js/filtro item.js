...
const filtroun = () => {
  const columns = [
    { name: 'Un_medida', index: 2, isFilter: true }
  ]
  const filterColumns = columns.filter(c => c.isFilter).map(c => c.index)
  const trs = document.querySelectorAll(`#tabela_dados tr:not(.header)`)
...
const filtroqtd = () => {
  const columns = [
    { name: 'quantidade', index: 3, isFilter: true }
  ]
  const filterColumns = columns.filter(c => c.isFilter).map(c => c.index)
  const trs = document.querySelectorAll(`#tabela_dados tr:not(.header)`)
...
const filtrocat = () => {
  const columns = [
    { name: 'categoria', index: 5, isFilter: true }
  ]
  const filterColumns = columns.filter(c => c.isFilter).map(c => c.index)
  const trs = document.querySelectorAll(`#tabela_dados tr:not(.header)`)
...
const filtrolote = () => {
    const columns = [
      { name: 'Lote', index: 6, isFilter: true }
    ]
    const filterColumns = columns.filter(c => c.isFilter).map(c => c.index)
    const trs = document.querySelectorAll(`#tabela_dados tr:not(.header)`)
...
const filtrovalidade = () => {
  const columns = [
    { name: 'validade', index: 7, isFilter: true }
  ]
  const filterColumns = columns.filter(c => c.isFilter).map(c => c.index)
  const trs = document.querySelectorAll(`#tabela_dados tr:not(.header)`)
